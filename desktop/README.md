...
- **Slice 5 (install snippets):** the **Install** screen gives copy-paste connect snippets for Claude
  Code, Codex (TOML), Cursor, Hermes, and a generic MCP agent — wrapping an MCP server with
  `@lyhna/mcp` (stdio quick-connect). Honest notes carry the two-surface story, the signed-mode
  data-egress disclosure, the witness-scope caveat, and "no one-command install / confirm your client's
  config in its docs." Snippets are generated by the pure, unit-tested `core/installSnippets.ts`.
- **Next slices:** exports/open-folder, adapter status, packaging.
