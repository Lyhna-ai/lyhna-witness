...
const REQUIRED_RULES = Object.freeze([
  ...
  ["delivery payload identity", /`review_delivered`[\s\S]*`report_ref`, `report_digest`, `audience`, `channel`, `delivery_ref`/],
  ["tool result pairing", /`tool_returned`[\s\S]*`request_event_ref`, `capture_status`/],
  ["closed-event disposition", /`review_closed`[\s\S]*`report_ref`, `report_digest`, `closing_review_ref`, `closing_subject`, `finding_dispositions`/],
  ["review transition linkage", /Every review transition resolves its required predecessor by explicit reference before it mutates review state\./],
  ["report identity continuity", /`review_available` resolves to a preceding `review_reported`[\s\S]*`review_delivered` resolves to that `review_available`[\s\S]*`review_acknowledged` resolves to that delivery/],
  ["repair finding resolution", /Every `repair_started\.finding_refs` entry resolves to the immutable finding set of the preceding report for the same `review_ref`/],
  ["canonical report digest preimage", /The report-digest preimage is exactly the RFC 8785 JSON Canonicalization Scheme \(JCS\) UTF-8 encoding of the entire resolved `lyhna-review-report\/v1` object at `report_ref`/],
  ["reported resource verification", /`review_reported` is accepted only after `report_ref` resolves[\s\S]*recompute to `report_digest`[\s\S]*ordered unique `finding_ref` projection[\s\S]*equals `finding_refs` exactly/],
  ["canonical supersession target", /`superseded_by` is exactly `\{ review_ref, subject \}`[\s\S]*complete canonical currentness subject/],
  ["closure finding-set equality", /`review_closed\.finding_dispositions` covers exactly the target report's finding set/],
  ["closing gate resolution", /`closing_review_ref` resolves to the stated current exact-head gate/],
  ["closing gate remains current", /`closing_review_ref` must still be the reducer-selected current, non-superseded review/],
  ["canonical input order", /The reducer consumes the validated event array as canonical input order and never[\s\S]*re-sorts it[\s\S]*strictly increasing/],
  ["deterministic current review selection", /For each complete canonical currentness subject with an accepted `review_started`, the review fold stores one `latest_review_ref` and one nullable `current_review_ref`[\s\S]*latest accepted `review_started` event in canonical input order sets both references/],
  ["same-subject implicit supersession", /When that pointer changes, the previously selected review becomes derived `superseded`[\s\S]*cannot authorize closure/],
  ["no clean-review fallback", /If the latest review is explicitly superseded without a same-subject replacement, `current_review_ref` becomes null and never falls back to an older review/],
  ["canonical closing subject", /`closing_subject` is exactly `\{ repository, head, review_scope, base, snapshot: \{ digest, coverage_digest \} \}`/],
  ["canonical report markdown digest", /`report_markdown_digest` is exactly `sha256:<64-lowercase-hex>` over the exact raw `REPORT\.md` bytes/],
  ["report markdown resolution", /Before `review_reported` is accepted, and again immediately before `review_available` or `review_delivered` is accepted, the sibling `REPORT\.md` bytes at `report_ref` must resolve and recompute to `report_markdown_digest`/],
  ["read-time report markdown resolution", /Every `read_review_report` operation resolves the sibling `REPORT\.md` immediately before returning bytes,[\s\S]*recomputes `report_markdown_digest`, and returns only those verified bytes\.[\s\S]*emits no `review_acknowledged`[\s\S]*only to the bytes verified by that same read/],
  ["report transition laundering fixture", /\*\*Report-transition laundering:\*\*[\s\S]*different report identity/],
  ["repair reference laundering fixture", /\*\*Repair-reference laundering:\*\*[\s\S]*unrelated review/],
  ["closure set laundering fixture", /\*\*Closure-set laundering:\*\*[\s\S]*stale closing gate/],
  ["report preimage laundering fixture", /\*\*Report-preimage laundering:\*\*[\s\S]*digest or finding set/],
  ["non-head supersession laundering fixture", /\*\*Non-head supersession laundering:\*\*[\s\S]*same head/],
  ["superseded closing gate laundering fixture", /\*\*Superseded-closing-gate laundering:\*\*[\s\S]*cannot authorize closure/],
  ["review selection laundering fixture", /\*\*Review-selection laundering:\*\*[\s\S]*newer finding-bearing review/],
  ["markdown digest encoding laundering fixture", /\*\*Markdown-digest encoding laundering:\*\*[\s\S]*uppercase or bare hexadecimal/],
  ["markdown bytes laundering fixture", /\*\*Markdown-bytes laundering:\*\*[\s\S]*missing or altered `REPORT\.md`/],
  ["read-time markdown laundering fixture", /\*\*Read-time Markdown laundering:\*\*[\s\S]*before `read_review_report`;[\s\S]*no Markdown bytes and emit no acknowledgement/],
