...
test("the structural validator rejects removal of identity and currentness rules", () => {
  ...
  for (const fragment of [
    ...
    "`review_reported` is accepted only after `report_ref` resolves",
    "`superseded_by` is exactly `{ review_ref, subject }`",
    ...
    "`closing_review_ref` must still be the reducer-selected current, non-superseded review",
    "The reducer consumes the validated event array as canonical input order and never",
    ...
    "When that pointer changes, the previously selected review becomes derived `superseded`",
    "If the latest review is explicitly superseded without a same-subject replacement, `current_review_ref` becomes null and never falls back to an older review",
    "`closing_subject` is exactly `{ repository, head, review_scope, base, snapshot: { digest, coverage_digest } }`",
    "`report_markdown_digest` is exactly `sha256:<64-lowercase-hex>` over the exact raw `REPORT.md` bytes",
    ...
    "**Report-preimage laundering:**",
    "**Non-head supersession laundering:**",
    "**Superseded-closing-gate laundering:**",
    "**Review-selection laundering:**",
    "**Markdown-digest encoding laundering:**",
    "**Markdown-bytes laundering:**",
    "**Read-time Markdown laundering:**",
