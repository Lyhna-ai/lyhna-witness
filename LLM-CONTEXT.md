...
**Lyhna Desktop app — v1 in-repo, feature-complete (2026-06-18):** a runnable Electron + Vite/React +
TypeScript app under `desktop/` (own `package.json`/`node_modules`; engine `src/` stays zero-dep). All
product logic is in a framework-agnostic, vitest-tested `desktop/core/`; the Electron main is a thin shell
that shells out to the engine CLIs (never re-implements receipt/capsule semantics). Slices merged: scaffold
(#52) · real inbox (#53) · receipt detail (#54) · sample-receipt flow (#55) · install snippets (#56) ·
exports/open-folder (#57) · adapter status (#58) · packaging config (electron-builder) + docs. Honesty held
throughout (no fake "Connected", samples labeled, signed-mode egress disclosed, "review before continuing"
not "blocked"). **Deferred:** standalone installer (needs the engine **bundled** — today it locates the
sibling engine via repo layout / `LYHNA_ENGINE_*` env), live adapter start/stop+detection, signing UI,
Settings pane, extraction to a `lyhna-desktop` repo, on-display visual QA. No prebuilt download / billing /
